...
/*  T6 C2 */    return await users.findOne({ "email": email }) 
  }
...
/*  T6 C2 */ await users.insertOne({ "name": userInfo.name, "email": userInfo.email, "password": userInfo.password },{w:1})
      return { success: true }
    } catch (e) {
...
/*  T6 C2 */ await sessions.updateOne(
        { user_id: email },
        { $set: { jwt: jwt } },
        { upsert: true }
      )
...
/*  T6 C2 */ await sessions.deleteOne({ "user_id": email })
      return { success: true }
    } catch (e) {
...
/*  T6 C2 */ return sessions.findOne({ "user_id": email })
    } catch (e) {
